...
class CalculationService:
    def calculate_posts(self, segment, coefficient, use_denominator=False):
        ...
        result = math.ceil(((segment.preform_operation_time + segment.form_operation_time + segment.postform_operation_time) * kh) / (segment.cycle_time - segment.transfer_time))
        if result >= 0:
            return result
        else:
